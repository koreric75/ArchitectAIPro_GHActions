...
def format_output(mermaid_code: str, repo_name: str, config: dict) -> str:
    ...
    return textwrap.dedent(f"""\
        # 🏗️ {org} — {repo_name} Architecture

        > **Created with [Architect AI Pro]({app_url})** — the flagship architecture tool by **{org}**
        > Auto-generated on {{{{ date }}}} | [GitHub Action source]({gh_url})

        ![BlueFalconInk LLC](https://img.shields.io/badge/{org.replace(' ', '%20')}-Standard-1E40AF)
        ...
        ## 📋 {org} Building Code Compliance

        | Standard | Requirement | Status |
        |----------|-------------|--------|
        | Cloud Provider | {config.get('technical_constraints', {}).get('preferred_cloud', 'AWS')} | ✅ Enforced |
        | IaC | {config.get('technical_constraints', {}).get('iac_tool', 'Terraform')} | ✅ Enforced |
        | Orchestration | {config.get('technical_constraints', {}).get('container_orchestration', 'Kubernetes')} | ✅ Enforced |
        | API Standard | {config.get('technical_constraints', {}).get('api_standard', 'GraphQL')} | ✅ Enforced |
        | Security Boundary | Required | ✅ Enforced |
        | WAF/ALB for Public | Required | ✅ Enforced |
        | Brand Identity | {org} | ✅ Enforced |

        ---

        ## 🏢 About

        This architecture diagram was generated by **[Architect AI Pro]({app_url})**, the flagship
        architecture tool built by **{org}**. Architect AI Pro analyzes your source code and
        produces compliant, production-ready architecture diagrams using Google Gemini AI.

        📎 **Live App:** [{app_url}]({app_url})
        📎 **GitHub Actions:** [{gh_url}]({gh_url})

        ---

        *© {org}. All rights reserved. Automated Governance. Living Blueprints. Ruthless Consistency.*
